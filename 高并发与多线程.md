...
​		悲观锁只对数据被外界修改持保守状态，认为数据很容易被其他线程修改，在数据处理前进行加锁，整个过程中处于加锁状态。悲观锁依赖数据库的锁机制，即在数据库中对当前操作加入排他锁。

## 乐观锁

​		乐观锁不会使用数据库提供的锁机制，一般在表中添加version字段或者使用业务状态来实现。乐观锁直到提交时才会锁定。
...
- 排他锁（互斥锁）
  ...
  ​	在读线程加读锁，在写线程加写锁，读可以同时读，不能写（大大提高了效率，不同一个读完了再读），写可以同时写，不能读
...
双向链表的作用，入队的时候需要看前面节点的状态，如果别取消直接跳过该节点
...
JUC包( java.util.concurrent)出来后加入了ConcurrentHashMap，写的效率降低，读的效率提升
...
没有ConcurrentTreeMap，因为CAS在树上太复杂，所以采用跳表来实现ConcurrentSkipListMap高并发并且排序。
