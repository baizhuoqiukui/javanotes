...
Future是一个可以取消异步计算的类。实现了Future接口，可以调用方法来开始和取消一个计算一个任务。

异步任务启动以后，可以获得一个绑定当前异步任务的FutureTask。

一般是配合Callable去使用。

可以基于FutrueTask的方法去取消任务，查看任务是否结束，查看任务返回结果。
...
```java
public static void main(String[] args) throws InterruptedException {
    // 构建FutureTask，基于泛型的返回结果类型
    // 在有参构造中，声明Callable或Runnable指定任务
    ...
    // futureTask提供了run方法，一般不会自己去调用run方法，让线程池去执行，由线程池去执行run方法
    // 此处和没执行run方法是一样的，因为任务是有状态的，任务已经执行了，再次执行run方法，不会再次执行任务
  	// 如果希望再次执行任务，可以调用runAndReset方法 protected方法
    futureTask.run();
  
    // 对任务状态的获取
    System.out.println("任务结束了吗？" +futureTask.isDone());
    Thread.sleep(1000);
    System.out.println("任务结束了吗？" +futureTask.isDone());
    Thread.sleep(1000);
    System.out.println("任务结束了吗？" +futureTask.isDone());
    ...
    //对返回结果的获取，类似阻塞队列的take方法
    try {
        String s = futureTask.get();
        System.out.println("任务返回结果：" + s);
    } catch (ExecutionException e) {
        throw new RuntimeException(e);
    }
  
  	// 对返回结果的获取，类似阻塞队列的poll方法
    // 如果超过设定的超时时间，会抛出异常，TimeOutException
    try {
      String s = futureTask.get(1000, TimeUnit.MILLISECONDS);
      System.out.println("任务返回结果：" + s);
    } catch (Exception e) {
      System.out.println("任务返回异常：" + e.getMessage());
      throw new RuntimeException(e);
    }
}
```
 ...
 * NEW -> COMPLETING -> NORMAL          任务执行正常，并且返回结果也正常返回
 * NEW -> COMPLETING -> EXCEPTIONAL 		任务执行正常，但是返回结果异常
 * NEW -> CANCELLED											任务被取消
...
public void run() {
    ...
    try {
        ...
        if (c != null && state == NEW) {
          	// 执行任务
            V result; // 任务返回结果
            boolean ran; // 异常标志 true任务正常结束 fasle任务异常结束
            try {
                // 执行业务run里的代码
                result = c.call();
                ran = true;
...
#### 1.3.3 set&setException方法

任务执行完毕后，修改任务的状态，封装任务的结果。
...
protected void set(V v) {
  	// 任务执行完毕，将任务的new状态，修改为completing状态
...
protected void setException(Throwable t) {
  	// 任务执行完毕，将任务的new状态，修改为completing状态 
    if (UNSAFE.compareAndSwapInt(this, stateOffset, NEW, COMPLETING)) {
      	// 将异常信息封装到了outcome属性当中
        outcome = t;
      	// 将任务状态变成EXCEPTIONAL 异常结束
...
#### 1.3.4 FutureTask 的cancel方法

任务取消的方式

1. 要么直接从new状态转换为cacel
2. 任务从new状态 -> interrupting -> interrupted

```java
public boolean cancel(boolean mayInterruptIfRunning) {
  	// 查看任务状态是不是new状态，正在执行中的任务也是new状态，基于传入的参数mayInterruptIfRunning 决定任务状态是从new转换为INTERRUPTING还是CANCELLED
...
private int awaitDone(boolean timed, long nanos)
    ...
    for (;;) {
        ...
        if (s > COMPLETING) {
      	...
      	// 任务状态COMPLETING 不需要阻塞线程，让步一下，等待一小会儿，结果就又了
...
#### 1.3.6 Future的finishCompletion方法

唤醒那些执行get方法等待任务结果的线程
...
private void finishCompletion() {
    // assert state > COMPLETING;
  	// 任务完成了需要唤醒阻塞的node 遍历链表
    for (WaitNode q; (q = waiters) != null;) {

      	// waitersOffset是waiters字段相对于this内存启始位置的偏移量
        // cas （waiters == q）直接将waiters置为null waitersOffset就是waiters变量
